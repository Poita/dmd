...
/* Counts of the functions whose code is generated, and of the template instance
 * functions whose code is left to other workers of a split compilation
 */
private __gshared size_t functionsGenerated;
private __gshared size_t functionsLeftToOtherWorkers;

/// Returns: true if `fd`, or a function it is nested in, belongs to a non-root module
private bool inNonRootFunction(FuncDeclaration fd)
{
...
/// Returns: true if `fd` is marked to have no code generated, which toSymbol() may do
private bool skipsCodegen(FuncDeclaration fd)
{
    toSymbol(fd);
    return fd.skipCodegen;
}
...
void FuncDeclaration_toObjFile(FuncDeclaration fd, bool multiobj)
{
    ...
    /* In a compilation split across worker processes, a template instance function
     * is generated by the worker that claims it first. With multiobj, it is claimed
     * before it is deferred to an object file of its own, unless it is not to be
     * generated, so that the other workers don't write such object files for it.
     */
    import dmd.mangle : mangleExact;
    import dmd.parallel : claimFunction;
    if (fd.isInstantiated())
    {
        const claim = multiobj && fd.semanticRun != PASS.semanticdone && !inNonRootFunction(fd) &&
            !skipsCodegen(fd);
        if (!claimFunction(mangleExact(fd).toDString(), claim))
        {
            ++functionsLeftToOtherWorkers;
            return;
        }
    }
    ...
    if (fd.skipCodegen) // test it again, as toSymbol() might have set it
        return;
    if (fd.isInstantiated() && !claimFunction(mangleExact(fd).toDString(), true))
    {
        ++functionsLeftToOtherWorkers;
        return;
    }
...
private void obj_write_deferred(ref OutBuffer objbuf, Library library, ref Dsymbols symbols_towrite)
{
    ...
    for (size_t i = 0; i < symbols_towrite.length; ++i)
    {
        ...
        const generated = functionsGenerated;
        const leftToOthers = functionsLeftToOtherWorkers;
        ...
        /* An object file with none of its functions, as other workers generate them,
         * is left out: they write the same object file
         */
        if (functionsGenerated == generated && functionsLeftToOtherWorkers != leftToOthers)
        {
            objmod.term(namebuf[]);
            objmod = null;
            objbuf.setsize(0);
